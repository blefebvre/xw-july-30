...
/**
 * Transformer: Stryker Dynamic Media / Scene7 images.
 *
 * The treatment-detail source serves images from media-assets.stryker.com — a
 * vanity CNAME of Scene7 — using /is/image/ IS/Image template URLs (verified in
 * migration-work/metadata.json .images.mapping: hero, brochure covers, and the
 * how-it-works step illustrations, each with $max_width_*$ / $max_width_auto$
 * template params). Detection is path-based (/is/image/), so the vanity
 * hostname is handled correctly.
 *
 * This transformer rewrites every DM <img> into an anchor so the DM URL (with
 * its template parameters intact) round-trips through docx -> markdown. A
 * companion auto-block in scripts/scripts.js (installed by the site-migration
 * orchestrator) rebuilds those anchors into responsive <picture> elements at
 * render time. Known gap: no width/height on the rebuilt <img> (CLS) — see the
 * dm-scene7-transformer reference. Ships without CLS handling in v1.
 *
 * Runs in afterTransform ONLY: block parsers run between the two hooks and
 * extract <img> references into block cells (cards-steps, cards-resource). If
 * this ran in beforeTransform those parsers would find no <img> and emit empty
 * image cells.
 *
 * Inlined helpers are copied byte-identical from the canonical
